...
export default {
  ...
  description: "Adds role options to message",
  ...
  callback: async ({ message, interaction, args, client }) => {
    ...
    if (targetMessage.author.id !== client.user?.id) {
      return `Please provide a message ID that was sent fomr <@${client.user?.id}>`;
    }
    ...
    if (menu) {
      for (const o of menu.options) {
        if (o.value === option[0].value) {
          return {
            custom: true,
            content: `<@&${o.value}> роль уже есть в этом меню`,
            allowedMentions: {
              roles: [],
            },
            ephemeral: true,
      ...
      row.addComponents(
        new MessageSelectMenu()
          ...
          .setMinValues(0)
          .setMaxValues(1)
          .setPlaceholder("Выбери о чем нам тебе напоминать...")
          .addOptions(option)
      );
    }
